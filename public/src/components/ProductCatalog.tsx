              ...
              >
                ...
                <div className="w-full md:w-[55%] bg-slate-950 flex flex-col justify-between overflow-y-auto max-h-[60vh] md:max-h-full border-r border-slate-800">
                  ...
                  <div className="relative bg-white flex items-center justify-center h-[360px] sm:h-[440px] md:h-[540px] lg:h-[620px] xl:h-[680px] p-8 select-none shrink-0 border-b border-slate-800">
                    ...
                    {selectedProduct.id !== 'prod-white-cabinet' && (
                      <div className="absolute bottom-4 right-4 bg-slate-950/80 backdrop-blur-sm px-3 py-1.5 rounded border border-slate-800 text-[11px] text-slate-300">
                        💡 아래 연출 예시 탭을 클릭하여 각 연출 사진을 확인해보세요
                      </div>
                    )}
                  </div>

                  {/* Interactive Example Selector Bar for Black Showcase */}
                  {selectedProduct.id !== 'prod-white-cabinet' && (
                    <div className="p-5 bg-slate-900/60 border-b border-slate-800 space-y-3">
                      ...
                      <div className="grid grid-cols-5 gap-2">
                        {PRESET_USES.filter(p => p.id !== 'white').map((preset) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-[10px] sm:text-[11px] font-medium leading-tight text-center">{preset.label.split(' ')[0]}</span>
                            </button>
                          );
                        })}
                      </div>
                  ...
                  {/* Detailed Example Photo Gallery for Black Showcase */}
                  {selectedProduct.id !== 'prod-white-cabinet' && (
                    <div className="p-6 bg-slate-950 border-b border-slate-800 space-y-6">
                      <div className="border-b border-slate-800/60 pb-3">
                        ...
                        <p className="text-xs text-slate-400 font-light mt-1.5 leading-relaxed">
                          블랙 진열장에 다양한 소장품을 디스플레이한 실제 연출컷입니다. 보관 목적에 따라 자유롭게 레이아웃을 구성하실 수 있습니다.
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {[
                          {
                            title: '🍷 고급 주류 & 와인 바 연출',
                            desc: '싱글몰트 위스키, 와인, 샴페인 등의 고급 보틀 디스플레이',
                            image: cabinetLiquor,
                            icon: '🍷'
                          },
                          {
                            title: '⚽ 스포츠 메모라빌리아 연출',
                            desc: '사인볼, 스포츠 용품, 컬렉션 굿즈 등의 전용 진열 연출',
                            image: cabinetSports,
                            icon: '⚽'
                          },
                          {
                            title: '🏆 명예 트로피 & 상패 연출',
                            desc: '골프 트로피, 감사패, 우승컵 등 기업 및 개인의 명예 기념관',
                            image: cabinetTrophies,
                            icon: '🏆'
                          },
                          {
                            title: '👾 피규어 & 수집품 연출',
                            desc: '한정판 피규어, 스태츄, 수집 인형 등의 정밀 먼지 차단 진열',
                            image: cabinetFigures,
                            icon: '👾'
                          }
                        ].map((ex, idx) => (
                          ...
                          >
                            <div className="bg-white h-64 sm:h-72 flex items-center justify-center p-3 relative overflow-hidden">
                              <img
                                src={ex.image}
                                alt={ex.title}
                ...
                <div className="w-full md:w-[45%] flex flex-col justify-between p-8 sm:p-10 md:p-12 overflow-y-auto max-h-[60vh] md:max-h-full">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                          ...
                          { title: '하단 밀폐 수납함 세팅', desc: '재고 및 전용 소품 보관 공간' },
                  ...
                  <div className="pt-8 mt-8 border-t border-slate-800">
                    <button
                      onClick={() => {
                        const quoteText = `[${selectedProduct.name} 견적 의뢰]\n높이 : 1800mm\n가로 : 550mm\n깊이 : 450mm\n선반 개수 : 4단\n선반 간격 : 균등 배분`;
                        onQuoteWithProduct(quoteText, selectedProduct.categoryLabel, selectedProduct.dimensions);
                        setSelectedProduct(null);
                        setActivePreviewImage(null);
                      }}
